...
# 실행 (방역 예시)
python3 ai공장짓기/runner/runner.py "클로드 방역 ai/manifest.yaml" \
  --handlers ai공장짓기/runner/adapters/pest_adapter.py \
  --input "클로드 방역 ai/test/temp_mock_input_임시.json" --trigger event
...
python3 "클로드 콘텐츠 ai/test/test_content_on_runner.py"   # 콘텐츠 공장 9건
```
