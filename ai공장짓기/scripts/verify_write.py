...
1. Projects/클로드 방역 ai/manifest.yaml, validate_manifest.py, HANDOFF.md) 재현됐다.
...
사용법:
  ...
  python3 verify_write.py --sentinel-check <md 파일 경로>
  python3 verify_write.py --sentinel-scan <루트 디렉토리>
...
sentinel 규칙 (2026-07-13, C1): 잘림은 항상 파일 맨 끝에서 발생하므로, 모든 md
파일의 마지막 비어있지 않은 줄을 `<!-- ok -->`로 고정한다. Edit/Write 도구로
수정한 직후에는 `--sentinel-check <경로>` 로 해당 파일 하나를 검사하고,
정기 점검(주간종합 등)에는 `--sentinel-scan <루트 디렉토리>` 로 전체 md를
훑어 sentinel 누락(FAIL) 파일을 모아 보고한다. sentinel 부재 = 잘림 의심으로
기계 판정한다 (사후 휴리스틱보다 저렴하고 확실함).
"""
import sys
import os
import argparse
...
def check_sentinel(path):
    """단일 md 파일의 마지막 비어있지 않은 줄이 SENTINEL인지 검사.
    반환: (passed: bool, last_line: str)"""
    with open(path, "r", encoding="utf-8", errors="replace") as f:
        text = f.read()
    last = last_nonempty_line(text)
    return last == SENTINEL, last


def find_md_files(root):
    for dirpath, dirnames, filenames in os.walk(root):
        # .git, .obsidian 등 숨김/메타 디렉토리는 스캔 제외
...
def sentinel_scan(root):
    """root 하위 모든 .md 파일을 훑어 sentinel 누락 파일 목록을 반환."""
    fails = []
    total = 0
    for path in find_md_files(root):
        total += 1
        passed, last = check_sentinel(path)
        if not passed:
            fails.append((path, last))
    return total, fails


def cmd_sentinel_check(path):
    if not os.path.isfile(path):
        print(f"[ERROR] 대상 파일이 없음: {path}")
        sys.exit(2)
    passed, last = check_sentinel(path)
    if passed:
        print(f"[PASS] {path} — sentinel 확인됨")
        sys.exit(0)
    else:
        print(f"[FAIL] {path} — 마지막 줄이 sentinel 아님 (실제: {last!r})")
        sys.exit(1)


def cmd_sentinel_scan(root):
    if not os.path.isdir(root):
        print(f"[ERROR] 대상 디렉토리가 없음: {root}")
        sys.exit(2)
    total, fails = sentinel_scan(root)
    for path, last in fails:
        print(f"[FAIL] {path} — 마지막 줄: {last!r}")
    print(f"[SUMMARY] 전체 {total}개 md 중 FAIL {len(fails)}개")
...
def main():
    ...
    parser.add_argument("target_path", nargs="?", help="검사할 실제 파일 경로")
    parser.add_argument("expected_path", nargs="?", help="의도한 최종 내용이 담긴 임시 파일 경로")
    parser.add_argument("--handoff", dest="handoff_path", default=None,
                         help="자동 복구 시 한 줄 기록할 HANDOFF.md 경로 (생략 시 기록 안 함)")
    parser.add_argument("--sentinel-check", dest="sentinel_check_path", default=None,
                         help="md 파일 1개의 마지막 줄이 sentinel(<!-- ok -->)인지만 검사 (PASS/FAIL)")
    parser.add_argument("--sentinel-scan", dest="sentinel_scan_root", default=None,
                         help="루트 디렉토리 하위 모든 md의 sentinel 여부를 훑어 FAIL 목록+요약 출력")
    args = parser.parse_args()

    if args.sentinel_check_path:
        cmd_sentinel_check(args.sentinel_check_path)
        return
    if args.sentinel_scan_root:
        cmd_sentinel_scan(args.sentinel_scan_root)
        return

    if not args.target_path or not args.expected_path:
        parser.error("target_path/expected_path 는 --sentinel-check 나 --sentinel-scan 을 쓰지 않을 때 필수입니다")
