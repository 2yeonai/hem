---
name: weekly-synthesis-v2
description: 매주 일요일 21:00 주간 종합 노트를 2. Areas/Claude 세션로그/에 저장 (모델: Sonnet 5)
---

모델: 이 작업은 반드시 Sonnet 5 모델로 실행한다. (7일치 로그를 종합·요약하는 정형화된 작업이며 근거 충돌이나 전략적 판단이 필요한 작업이 아니므로 Fable 5 등 고비용 모델로 에스컬레이션하지 말 것.)
