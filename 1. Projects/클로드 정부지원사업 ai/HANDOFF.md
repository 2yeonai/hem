...
- **T10**: bizinfo API 키(`X9HC6x`) 발급 확인 완료, `config.yaml` 저장. 실응답 검증은 실제 실행 환경(혜미 PC+작업 스케줄러) 필요.
...
**다음 할 일**: ①bizinfo 키 재확인 후 매일 07:00 스케줄 등록 ②PPT초안 앵커 절단 정제(안내문·개인정보동의서 혼입 제거) ③PPT/대본/QNA 실 LLM 연동(QNA는 "5개 만능답변 템플릿" 구조로 재설계 필요) ④run.py 버전 헤더 정리(v0.5.4→반영 안 됨) ⑤홍재우 나머지 15개 판단기준.
