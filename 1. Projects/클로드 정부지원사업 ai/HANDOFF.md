...
## 매일 공고수집 이어받기 보완 (2026-07-21, [eunoia9496] GPT/Codex)

- **혜미 운영 기준**: 거의 매일 실행하므로 첫 수집 이후에는 과거 공고를 새 후보로 다시 만들지 않고, 지난 성공 수집 뒤 새로 발견된 공고만 후보 단계로 넘긴다. 하루를 건너뛰면 건너뛴 기간의 새 공고도 함께 잡는다.
- **안전한 기준**: 기업마당 공식 API에는 등록일 시작·종료 요청값이 없으므로 날짜로 잘라 누락 위험을 만들지 않는다. API 최신 목록을 확인한 뒤 과거 raw 파일의 `pblancId`와 비교해 처음 보는 공고만 남기는 기존 방식을 유지한다.
- **버그 수정**: 첫 `신규 0건` 실행에서 빈 candidate 파일을 만들어 이전 검수 목록이 사라져 보일 수 있던 흐름을 차단. 감사용 빈 raw는 남기되 exit code 10으로 candidate 승격을 건너뛰어 이전 후보 목록을 유지한다.
- **검증**: 합성 기존 공고 재등장 → return 10·신규 0건, 합성 새 공고 → return 0·신규 1건, `py_compile` 통과. 기존 마감일 파싱·검수·저장 구조는 변경하지 않음.
