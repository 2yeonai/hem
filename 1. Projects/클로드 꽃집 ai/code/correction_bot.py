...
_SYSTEM_PROMPT = """당신은 꽃집(온천꽃식물원) 주문 문자/통화 원문을 다듬는 보정 담당자입니다.
STT(음성인식) 오인식이나 구어체 표기를 문맥으로 보정한 normalized_text를 만드세요.

절대 원칙(반드시 지킬 것 - 어길 경우 실서비스 사고로 이어짐):
- 의미가 바뀌지 않는 명백한 표기 오류(예: "10마넌" -> "10만원")만 normalized_text에
  직접 반영하세요.
- 조금이라도 확신이 안 서는 보정(고유명사 오인식 추정, 화자 스스로 불확실("몰라",
  "글쎄", "아마" 등)을 표현한 부분, 서로 다른 이벤트 키워드가 동시에 등장하는 경우 등)은
  절대 normalized_text에서 원문을 바꾸지 말고, candidates 목록에만 후보로 남기세요.
- 원문에 없는 내용을 지어내지 마세요.

반드시 아래 JSON 형식으로만 답하세요:
{
  "normalized_text": "<보정된 텍스트, 명백한 것만 반영>",
  "correction_log": [{"original": "...", "corrected": "...", "type": "...", "note": "..."}],
  "candidates": [{"phrase": "...", "candidates": ["..."], "note": "왜 확신할 수 없는지"}],
  "confidence": <0.0~1.0>,
  "reason": "<한 문장 이유>"
}
"""
...
def _correct_llm(text: str) -> dict:
    ...
    if not isinstance(result.get("normalized_text"), str):
        raise llm_client.LLMUnavailableError(f"normalized_text 형식이 이상함: {result.get('normalized_text')!r}")
    result.setdefault("correction_log", [])
    result.setdefault("candidates", [])
    return result


def _correct_auto(text: str) -> dict:
    """LLM 우선, 실패 시 규칙기반 자동 대체."""
    try:
        return _correct_llm(text)
    except llm_client.LLMUnavailableError as e:
        result = dict(_correct_rule_based(text))
        result["reason"] = result["reason"] + f" [LLM 호출 실패로 규칙기반 대체: {e}]"
        return result


# 2026-07-17부터 LLM(Claude) 우선, 실패 시 규칙기반 자동 대체.
CORRECTION_ENGINE = _correct_auto
