...
kind별 처리 방식 (2026-07-14 갱신 — kind:model 6개 전부 규칙기반으로 구현 완료):
  - kind: local        → code/<stage_id>.py의 실제 함수를 호출한다(진짜 로직).
  - kind: model (전체 6개: order_classification_bot, order_split_bot, correction_bot,
    order_draft_bot, ribbon_price_bot, review_manager_bot) → 전부 규칙기반(정규식/사전/
    휴리스틱) 임시 구현을 실제로 호출한다. 실제 LLM 호출은 아직 없다(이 작업 환경에
    Claude API 키가 연결돼 있지 않음) — 각 파일 docstring에 known limitation이 명시돼
    있으니 그대로 참고할 것. 값을 임의로 지어내지 않는다는 프로젝트 원칙(불확실하면
    None + missing_fields/후보로 남김)은 6개 전부 동일하게 지킨다.
...
def run_case(case, db_path, verbose=True):
    ...
    for seg in split["order_segments"]:
        ...
        correction_result = correction_bot.correct_text(seg["segment_text"])
        normalized_text = correction_result["normalized_text"]
        correction_log = correction_result["correction_log"]
        candidates = correction_result["candidates"]
        emit(
            "[correction_bot] 정규화 완료(단위정규화 " + str(len(correction_log))
            + "건, 불확실 후보 " + str(len(candidates)) + "건 - 규칙기반, 낯선 오인식은 못 잡음/알려진 한계)"
        )

        draft_result = order_draft_bot.build_draft(normalized_text)
        order_draft = draft_result["order_draft"]
        missing_fields = draft_result["missing_fields"]
        emit(
            "[order_draft_bot] 필드 " + str(len(order_draft) - len(missing_fields))
            + "/" + str(len(order_draft)) + "개 채움 (missing=" + str(missing_fields) + ")"
        )

        ribbon_result = ribbon_price_bot.process_ribbon_and_price(order_draft, normalized_text)
        emit(
            "[ribbon_price_bot] product_name=" + str(ribbon_result["product_name"])
            + ", price=" + str(ribbon_result["price"])
            + ", ribbon_message_final=" + str(ribbon_result["ribbon_message_final"])
        )

        review_result = review_manager_bot.build_review(
            order_draft=order_draft,
            field_confidence=draft_result["field_confidence"],
            field_sources=draft_result["field_sources"],
            missing_fields=missing_fields,
            candidates=candidates,
            ribbon_message_raw=ribbon_result["ribbon_message_raw"],
            ribbon_message_final=ribbon_result["ribbon_message_final"],
            product_name=ribbon_result["product_name"],
            price=ribbon_result["price"],
            quantity=ribbon_result["quantity"],
            correction_log=correction_log,
            split_status=split["split_status"],
        )
        emit(
            "[review_manager_bot] review_priority=" + review_result["review_priority"]
            + ", checklist 항목 " + str(len(review_result["review_checklist"])) + "건"
        )

        _mock_human_stage(emit, "human_reviewer")
        confirmed_fields = dict(order_draft)
        confirmed_fields["normalized_text"] = normalized_text
        confirmed_fields["ribbon_message_raw"] = ribbon_result["ribbon_message_raw"]
        confirmed_fields["ribbon_message_final"] = ribbon_result["ribbon_message_final"]
        confirmed_fields["product_name"] = ribbon_result["product_name"]
        confirmed_fields["price"] = ribbon_result["price"]
        confirmed_fields["quantity"] = ribbon_result["quantity"]
        manual_edits = {}
