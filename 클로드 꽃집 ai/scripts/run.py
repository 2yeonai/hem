...
v0.4.0 변경사항 (실제 공고문 PDF + 실제 사업자 프로필로 테스트하며 발견된 문제 중 3건 수정):
  ...
  - [심사배점 매칭] judge_mode_self_review의 rubric 커버리지 체크를 "항목명 첫 단어만 포함 검사"에서
    ...
    주의: 첫 시도에서는 "토큰 앞 2글자가 본문에 있으면 매칭"이라는 규칙을 썼다가 "창업"/"사업"처럼
    아주 흔한 어근 때문에 거의 모든 항목이 과도하게 "커버됨"으로 판정되는 문제를 발견해 제거함.
    지금은 동의어 테이블에 등록된 어근만 부분일치를 허용하고, 그 외 단어는 완전일치만 인정한다 —
    여전히 의미 기반 매칭은 아니므로 최종 판단은 사람이 해야 한다.
...
def check_eligibility_and_disqualification(profile, announcement, ref_date):
    ...
    if industry_codes and industry_codes != "any":
        ...
        if val is None:
            ...
            criteria.append(("업종코드", val in industry_codes))
...
# ---------------------------------------------------------------------------
def match_programs(profile, announcements, target_period, ref_date):
    matched = []
    excluded = []
    needs_confirmation = []
    ...
    for a in announcements:
        ...
        criteria, disq_risks, unverifiable = check_eligibility_and_disqualification(profile, a, ref_date)
...
def main():
    # 사용법: python3 run.py [input.json] [announcements.json]
