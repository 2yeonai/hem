...
6. **문서명 fuzzy 매칭 미지원.** documents_status의 document_name이 공고 required_documents 문자열과 정확히 일치해야 매칭된다. `my_business_profile_input.json`/`real_program_input_simulated.json`에서 PSST-Support가 fail로 나오는 것도 이 known limitation 때문(실제로는 서류가 있어도 표기가 달라 미준비로 집계됨).
...
- 실제 공고 3번째("소상공인 생활문화 혁신지원" 등, gov-support-skill/ 폴더에 PDF 이미 존재)도 필요 시 추가 파싱
